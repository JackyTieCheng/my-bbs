...
public class TempletesInterceptor implements HandlerInterceptor{
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, 
		...
		//	if(mav.matches("^templates/.*$")){
			...
			if(mav != null && !"".equals(mav) && mav.length() >11){
				String begin = mav.substring(0, 10);//取得templates
				if("/templates".equals(begin) //如果以templates开头
						&& !StringUtils.startsWith(mav, "forward:/common/")){//判断开始部分是否与二参数相同    forward:/common/default/pc/images/favicon.ico
